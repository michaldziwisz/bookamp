...
INT_PTR CALLBACK MainProc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_DESTROY:
        g_dlg = nullptr;
        PostQuitMessage(0);   // konczy petle komunikatow watku GUI
        return TRUE;
    }
    return FALSE;
}

// --- Okno GUI zyje we WLASNYM watku z wlasna petla komunikatow. ---
// Powod: modeless dialog w cudzej petli Winampa nie dostaje IsDialogMessage
// (psuje nawigacje strzalkami/Tab) a DestroyWindow z obcego watku zawiesza Winampa.
...
static DWORD WINAPI guiThread(LPVOID) {
    ...
    HWND dlg = CreateDialogParamW(g_hInst, MAKEINTRESOURCEW(IDD_MAIN), g_parent, MainProc, 0);
    if (!dlg) return 1;
    ShowWindow(dlg, SW_SHOW);

    MSG msg;
    while (GetMessage(&msg, nullptr, 0, 0) > 0) {
        if (!IsDialogMessage(dlg, &msg)) {   // <-- naprawia strzalki/Tab
...
HWND createMainWindow(HINSTANCE hInst, HWND parent) {
    // Sprzatnij po poprzednim oknie: jesli watek GUI juz sie zakonczyl
    // (user zamknal okno), zwolnij uchwyt, by mozna bylo otworzyc ponownie.
    if (g_thread) {
        if (WaitForSingleObject(g_thread, 0) == WAIT_OBJECT_0) {
            CloseHandle(g_thread); g_thread = nullptr; g_threadId = 0; g_dlg = nullptr;
        } else {
            if (g_dlg) SetForegroundWindow(g_dlg);   // wciaz otwarte -> uwidocznij
            return g_dlg;
        }
    }
    g_hInst = hInst; g_parent = parent;
    g_thread = CreateThread(nullptr, 0, guiThread, nullptr, 0, &g_threadId);
    return g_dlg;
}

void destroyMainWindow() {
    if (g_thread) {
        if (g_dlg) PostMessage(g_dlg, WM_CLOSE, 0, 0);       // zamknij w JEGO watku
        WaitForSingleObject(g_thread, 2000);                 // poczekaj na zakonczenie petli
